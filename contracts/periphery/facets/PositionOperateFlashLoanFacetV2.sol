...
contract PositionOperateFlashLoanFacetV2 is PancakeFlashLoanFacetBase {
  ...
  constructor(address _pancakePoolV3, address _pancakePoolV2, address _poolManager, address _fxUSD) PancakeFlashLoanFacetBase(_pancakePoolV3, _pancakePoolV2) {
  ...
  function openOrAddPositionFlashLoan(
    ...
    _invokeFlashLoan(
      borrowAmount,
      abi.encodeCall(
        PositionOperateFlashLoanFacetV2.onOpenOrAddPositionFlashLoan,
        (pool, positionId, amountIn, borrowAmount, 0, msg.sender, data)
      )
    );
  ...
  function closeOrRemovePositionFlashLoan(
    ...
    _invokeFlashLoan(
      borrowAmount,
      abi.encodeCall(
        PositionOperateFlashLoanFacetV2.onCloseOrRemovePositionFlashLoan,
        (pool, positionId, amountOut, borrowAmount, 0, msg.sender, data)
      )
    );
  ...
  function onOpenOrAddPositionFlashLoan(
    ...
    LibRouter.approve(IPool(pool).collateralToken(), poolManager, amount - fee);
    position = IPoolManager(poolManager).operate(pool, position, int256(amount - fee), int256(fxUSDAmount));
  ...
  function onCloseOrRemovePositionFlashLoan(
    ...
    if (fxUSDAmount >= maxFxUSD) {
      ...
      IPoolManager(poolManager).operate(pool, position, -int256(amount + fee), -int256(fxUSDAmount));
      _checkPositionDebtRatio(pool, position, miscData);
    }
    IERC721(pool).transferFrom(address(this), recipient, position);
